...
export default function AllowancePage() {
  const [activeTab, setActiveTab] = useState<"detail" | "period" | "setting">("detail");
  ...
  useEffect(() => {
    if (activeTab === "period") {
      ...
      fetchDetails();
    } else if (activeTab === "setting" && roleId === 3) {
      fetchSettings();
    }
  }, [activeTab, roleId]);
  ...
  const fetchPeriods = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.message || "Failed to fetch period data");
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.message || "Failed to fetch detail data");
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6">
                ...
                <table className="w-full text-sm text-left text-slate-600 whitespace-nowrap">
                  <thead className="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3">Nama Penerima</th>
                ...
                <table className="w-full text-sm text-left text-slate-600">
                  <thead className="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-center">Total Penerima</th>
                      <th scope="col" className="px-6 py-3 text-center">Total Tunjangan Transport</th>
                      <th scope="col" className="px-6 py-3 text-center">Status</th>
                    ...
                    {isLoading ? (
                      ...
                      periods.map((period, index) => (
                        <tr key={period.id} className="bg-white border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 font-medium text-slate-900">
                            {new Date(2000, period.period_month - 1).toLocaleString('en-US', { month: 'long' })}
                          </td>
                          <td className="px-6 py-4 text-center">
          ...
          {/* --- PAGE SETTING TAB --- */}
          {activeTab === "setting" && canViewSetting && (
            <div className="animate-in fade-in duration-300">
                  ...
                  <tbody>
                    {isLoading ? (
                      ...
                      settings.map((setting) => (
                        <tr key={setting.id} className="bg-white border-b border-slate-100 hover:bg-slate-50">
                          <td className="px-6 py-4 font-medium text-slate-900">
                            {setting.id}
                          </td>
                          <td className="px-6 py-4 font-medium text-slate-900">
